...
from modules.linkextractor import extract_links
from modules.directoryscan import directory_scan
from modules.sqlinjection import test_sqli
from modules.xsstest import test_xss
from modules.portscanner import port_scan, run_nmap
from modules.subdomainscanner import scan_subdomains
...
if __name__ == "__main__":
    try:
        ...
        use_subs  = input("  Run subdomain scan? (Y/N): ").strip().lower() == "y"
        use_ports = input("  Run port scan? (Y/N): ").strip().lower() == "y"
        ...
        if save_json:
            ...
            filename = os.path.join(
                reports_dir,
                f"scan_{urlparse(target).netloc}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
            )
